...
public:
    ...
    void deploy(const Bool& forced = true) {
        ...
        for (auto& path: m_dependencies) {
            ...
            Path::remove(dest.join(toString(full_name, ".xcodeproj")));
        ...
        if (proc.execute(toString("/opt/homebrew/bin/heroku git:remote -a ", m_app)) != 0) {
            throw HerokuError("vweb::utils::Heroku", toString("Failed to set the heroku git remote of app \"", m_app, "\"."));
        ...
        if (proc.execute("/opt/homebrew/bin/heroku buildpacks:set https://github.com/vandenberghinc/vweb-buildpack.git") != 0) {
            throw HerokuError("vweb::utils::Heroku", toString("Failed to deploy \"", m_app, "\"."));
        } else if (proc.exit_status() != 0) {
            String output = proc.err_or_out();
            if (!output.contains("is already set")) {
                throw HerokuError("vweb::utils::Heroku", toString("Failed to set the buildpack of app \"", m_app, "\": \n", output));
            }
        }
        ...
        if (proc.execute(toString("/opt/homebrew/bin/heroku config:set ", env_str)) != 0) {
            throw HerokuError("vweb::utils::Heroku", toString("Failed to deploy \"", m_app, "\"."));
        } else if (proc.exit_status() != 0) {
            throw HerokuError("vweb::utils::Heroku", toString("Failed to set the environment variables of app \"", m_app, "\": \n", proc.err_or_out()));
        }
        
        // Push.
        proc.log = true;
        if (proc.execute(toString("cd ", m_source, " && git add -A && git commit -am \"Automatic deploy\" && git push heroku HEAD:master", (forced ? " -f" : ""))) != 0) {
            throw HerokuError("vweb::utils::Heroku", toString("Failed to deploy \"", m_app, "\"."));
        }
