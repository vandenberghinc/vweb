...
const initialize_server = (path = null, source = "") => {
    ...
    if (path.extension() == ".js") {
        ...
        if (config instanceof vweb.Server) {
...
const cli = new vlib.CLI({
    name: "vweb",
    description: `The vweb cli. The cli must be able to access the initialized server instance or its configuration object. This can be achieved in two ways.
...
`,
    ...
    commands: [
         ...
         */
        {
            ...
            callback: (_a) => __awaiter(void 0, [_a], void 0, function* ({ source = null, config = null, daemon = false, }) {
                const { path, server } = initialize_server(config, source);
                // Service daemon.
                if (daemon) {
                    // Server daemon.
                    if (server.db.daemon) {
                        yield server.db.daemon.stop();
                        if (server.db.daemon.exists()) {
                            yield server.db.daemon.update();
                        }
                        else {
                            yield server.db.daemon.create();
                        }
                        yield server.db.daemon.start();
                    }
                    // Server daemon.
                    if (!server.daemon) {
                        cli.throw_error("The server service daemon is disabled.");
                    }
                    yield server.daemon.stop();
                    if (server.daemon.exists()) {
                        yield server.daemon.update();
                    }
                    else {
                        yield server.daemon.create();
                    }
                    yield server.daemon.start();
                }
                // Development server.
                else {
         ...
         */
        {
            ...
            callback: (_a) => __awaiter(void 0, [_a], void 0, function* ({ source = null, config = null, }) {
