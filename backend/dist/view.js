...
class View {
    // Constructor.
    constructor({ source = null, callback = null, includes = [], links = [], templates = {}, meta = new Meta(), jquery = false, compression = false, payments = false, vhighlight = false, lang = "en", body_style = null, splash_screen = null, tree_shaking = false, mangle = false, _src = __filename, }) {
        // Arguments.
        this.source = source;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.links.iterate((url) => {
                if (typeof url === "string") {
                    ...
                    // Embed content.
                    if ((typeof url === "object" && url.rel === "stylesheet" && url.embed !== true) &&
                        embed_stylesheet(utils.clean_endpoint(url.href))) { /* skip */ }
                    // Create link.
                    else {
                        if (url.async) {
            ...
            // Add vweb static aspect ratios.
            embed_cached_code();
            this.html += `<script>vweb.static.aspect_ratios = ${JSON.stringify(server.statics_aspect_ratios)}</script>`;
            // Embed other scripts.
            if (this.vhighlight) {
...
View.includes = [];
View.links = [];
View.body_style = null; // css string style.
View.splash_screen = null; // SplashScreen object.
// ---------------------------------------------------------
// Exports.
module.exports = View;
