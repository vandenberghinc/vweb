...
const { StaticFileWatcher, FileWatcher } = require("./file_watcher.js");
...
class Server {
    // Constructor.
    constructor({ ip = "127.0.0.1", port = 8000, domain = null, is_primary = true, source = null, database = "mongodb://localhost:27017/main", statics = [], favicon = null, company = {}, meta = new Meta(), tls = null, smtp = null, mail_style = {
    ...
    }, keys = [], payments = null, default_headers = null, google_tag = null, token_expiration = 86400, enable_2fa = false, enable_account_activation = true, honey_pot_key = null, production = false, multiprocessing = true, processes = null, file_watcher = {}, offline = false, additional_sitemap_endpoints = [], log_level = 0, daemon = {}, admin = {
        password: null,
        ips: [],
    }, }) {
        // @debug
        ...
        if (default_headers === null) {
            ...
            this.default_headers = Object.assign({}, base_default_headers);
        }
        else {
        ...
        if (payments) {
            if (payments.type === "adyen") {
                this.payments = new Adyen(Object.assign({ _server: this }, payments));
            }
            else if (payments.type === "paddle") {
                this.payments = new Paddle(Object.assign({ _server: this }, payments));
            }
            else {
        ...
        // No file watcher.
            ...
            if (typeof database === "string" || database instanceof String) {
                ...
                this.db = new Database(Object.assign(Object.assign({}, database), { _server: this }));
            }
            ...
            if (this.is_primary) {
                this.rate_limit = new RateLimitServer(Object.assign(Object.assign({}, rate_limit.server), { _server: this }));
            }
            else {
                if (rate_limit.server.https) {
                    rate_limit.client.https = true;
                }
                this.rate_limit = new RateLimitClient(Object.assign(Object.assign({}, rate_limit.client), { _server: this }));
            }
            // Blacklist class.
    ...
    */
    ...
    _create_default_endpoints() {
        ...
        this.endpoint(new Endpoint({
            ...
            callback: (stream, params) => __awaiter(this, void 0, void 0, function* () {
                ...
                // Load data.
                const data = yield this._sys_db.load("status", {
                    default: {
                        running_since: null,
                        running_threads: 0,
                        total_threads: 0,
    ...
    _create_admin_endpoint() {
        ...
        this.endpoint(new Endpoint({
            ...
            callback: (stream, params) => __awaiter(this, void 0, void 0, function* () {
                ...
                const data = {};
                // Parse subscriptions.
                const subscriptions = yield this.payments._get_all_active_subscriptions();
                data.subscriptions = subscriptions.length;
                // Load data.
                const status = yield this._sys_db.load("status", {
                    default: {
                        running_since: null,
                        running_threads: 0,
                        total_threads: 0,
                ...
                data.network_usage = yield vlib.system.network_usage();
                // Users.
                data.users = (yield this.users.list()).length;
                // Response.
                return stream.send({
                    status: 200,
                    headers: { "Content-Type": "application/json" },
                ...
                callback: () => {
                    ...
                    const SingleStatWidget = (title, stat, color = null, font_size = 35) => {
                        ...
                        return Widget(Title(title)
                            // .color(style.sub_fg)
                            .font_size(14)
                            .font_weight(600), VStack(Text(stat)
                    ...
                    // View.
                    const VIEW = View(VStack(HStack(ImageMask("/vweb_static/admin/admin.png")
                    ...
                    Title("Admin Panel")
                        ...
                        .font_size(14)
                        .margin_left(5)
                        .margin_top(3), Spacer(), Text("$DOMAIN")
                        .font_size(14)
                        .color(style.fg)
                    // .font_style("italic")
                    )
                        ...
                        .box_shadow("0 0 10px #00000050"), VStack(
                    // Auth widget.
                    (AUTH_WIDGET = Widget(Title("Authentication Required")
                        .font_size(28)
                        .margin_bottom(10), Text("Fill in your admin authentication password to visit the admin panel."), ExtendedInput({
                        label: "Password",
                        ...
                        .id("password")
                        .margin_top(35)
                        .on_enter(() => document.getElementById("auth_submit").click()), LoaderButton("Authenticate")
                        .loader.background(style.sub_fg).parent()
                        .margin_top(35)
                        .id("auth_submit")
                        .on_click((button) => __awaiter(this, void 0, void 0, function* () {
                        const input = document.getElementById("password");
                        const password = input.submit();
                        button.show_loader();
                        try {
                            const response = yield vweb.utils.request({
                                method: "POST",
                                url: "/admin/auth",
                                data: {
                                    password: password,
                    ...
                    (PANEL_WIDGET = VStack()
                        ...
                        .center()
                        .hide()
                        .assign("load", function () {
                        return __awaiter(this, void 0, void 0, function* () {
                            // Show.
                            this.show();
                            this.remove_children();
                            this.append(VStack(RingLoader()
                                .frame(30, 30)
                            ...
                            const data = yield vweb.utils.request({
                                method: "GET",
                                url: "/admin/data",
                                data: {
                                    token: TOKEN,
                            ...
                            // Show.
                            this.remove_children();
                            this.append(VStack(Title("Admin Panel")
                                .font_size(32)
                                .margin(0, 0, 10, 10), Title("Status")
                                .font_size(22)
                                .margin(20, 0, 0, 10), Divider()
                                .background(style.border)
                                .margin(10, 0, 10, 10), HStack(SingleStatWidget("Running Since", new vweb.Date(data.running_since).format("%d-%m-%y %H:%M"), null, 30), SingleStatWidget("Status", data.running_threads === data.total_threads ? "OK" : "Warnings", data.running_threads === data.total_threads ? null : "red", 30), SingleStatWidget("Threads", data.running_threads), SingleStatWidget("Crashed Threads", data.total_threads - data.running_threads, data.total_threads - data.running_threads === 0 ? null : "red"))
                                .wrap(true)
                                .align_height(), Title("System")
                                .font_size(22)
                                .margin(20, 0, 0, 10), Divider()
                                .background(style.border)
                                .margin(10, 0, 10, 10), HStack(SingleStatWidget("CPU Usage", data.cpu_usage.toFixed(2) + "%", data.cpu_usage > 90 ? "red" : null), SingleStatWidget("Memory Usage", data.memory_usage.used_percentage.toFixed(2) + "%", data.memory_usage.used_percentage > 90 ? "red" : null), SingleStatWidget("Network Sent", data.network_usage.sent), SingleStatWidget("Network Received", data.network_usage.received))
                                .wrap(true)
                                .align_height(), Title("Users")
                                .font_size(22)
                                .margin(20, 0, 0, 10), Divider()
                                .background(style.border)
                                .margin(10, 0, 10, 10), HStack(SingleStatWidget("Users", data.users), SingleStatWidget("Subscriptions", data.subscriptions))
                                .wrap(true)
                                .align_height())
                                .leading()
                                .frame("100%", "100%")
    ...
    _initialize_statics() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const static_paths = [];
            // Add static file.
            const add_static_file = (path_1, endpoint_1, ...args_1) => __awaiter(this, [path_1, endpoint_1, ...args_1], void 0, function* (path, endpoint, cache = true) {
                // Add to static paths.
                ...
                if (ImageEndpoint.supported_images.includes(path.extension())) {
                    ...
                    const aspect_ratio = yield e.get_aspect_ratio();
                    if (aspect_ratio != null) {
                        this.statics_aspect_ratios[e.endpoint] = aspect_ratio;
                    }
                    this.endpoint(e);
                }
                // Default static endpoint.
            ...
            // Initialize statics.
            const add_static = (opts) => __awaiter(this, void 0, void 0, function* () {
                if (opts == null) {
                    return;
                }
                    ...
                    // Not a directory.
                    if (!is_dir) {
                        return yield add_static_file(source, opts.endpoint, opts.cache);
                    }
                    // First extract all paths recursively.
                    // non recursive to ignore .old etc dirs.
                    const read_dir = (path) => __awaiter(this, void 0, void 0, function* () {
                        const dir_paths = yield path.paths();
                        const promises = [];
                    ...
                    if (is_dir) {
                        yield read_dir(source);
                    }
                    // Convert paths into a static object.
                    for (const path of paths) {
                        const endpoint = `${opts.endpoint}${path.str().substr(source_len)}`;
                        yield add_static_file(path, endpoint, opts.endpoint_cache[endpoint] == null ? opts.cache : opts.endpoint_cache[endpoint]);
                    }
                }
                else if (typeof opts === "string") {
                    yield add_static({ path: opts });
                }
            });
            // Iterate.
            for (let i = 0; i < this.statics.length; i++) {
                ...
                yield add_static(this.statics[i]);
            }
            // Response.
            return static_paths;
        });
    ...
    initialize() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.is_file_watcher) {
                ...
                this.statics.iterate_append(item => {
            ...
            if (this.db) {
                ...
                // Load keys.
                const keys_document = yield this._sys_db.load("keys");
                ...
                if (keys_document == null) {
                    ...
                    yield this._sys_db.save("keys", doc);
                }
                else {
                    // Check hash key.
                    ...
                    // Save.
                    if (perform_save) {
                        yield this._sys_db.save("keys", keys_document);
                    }
                }
            ...
            // Create static endpoints.
            yield this._initialize_statics();
            ...
            for (const endpoint of this.endpoints.values()) {
                yield endpoint._initialize(this);
            }
            ...
            if (this.payments !== undefined) {
                yield this.payments._initialize();
            }
    ...
    _serve(stream, headers, req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const serve_error_endpoint = (status_code) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (!this.err_endpoints.has(status_code)) {
                        ...
                        try {
                            yield err_endpoint._serve(stream, status_code);
                        }
                        catch (err) {
                ...
                if (this.online && this.blacklist !== undefined && !this.blacklist.verify(stream.ip)) {
                    yield serve_error_endpoint(403);
                    log_endpoint_result();
                    return;
                }
                ...
                if (!endpoint) {
                    ...
                    // Respond with 404.
                    yield serve_error_endpoint(404);
                    log_endpoint_result();
                    return;
                }
                // Check rate limit.
                if (this.online && endpoint.rate_limits.length > 0) {
                    const result = yield this.rate_limit.limit(stream.ip, endpoint.rate_limits);
                    if (result !== null) {
                        stream.send({
                            status: 429,
                            headers: {
                ...
                try {
                    ...
                    yield serve_error_endpoint(500);
                    log_endpoint_result();
                    return;
                }
                try {
                    ...
                    logger.error(`${endpoint.method}:${endpoint.endpoint}: `, err);
                    yield serve_error_endpoint(400);
                    log_endpoint_result();
                    return;
                }
                // Set default headers.
                ...
                if (!endpoint.is_static || endpoint.authenticated) {
                    ...
                    const auth_result = yield this.users._authenticate(stream);
    ...
    start() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.is_file_watcher) {
                ...
                yield this.file_watcher.promise;
                return null;
            }
            ...
            if (this.production && this.multiprocessing && libcluster.isMaster) {
                ...
                // Save status.
                yield this._sys_db.save("status", {
                    running_since: Date.now(),
                    total_threads: active_threads,
                    running_threads: active_threads,
                });
                // On exit.
                libcluster.on('exit', (worker, code, signal) => __awaiter(this, void 0, void 0, function* () {
                    // Fetch thread id.
                    ...
                    // Reached limit, shutdown thread.
                        ...
                        --active_threads;
                        yield this._sys_db.save("status", { running_threads: active_threads });
                        if (active_threads === 0) {
    ...
    */
    status() {
        return __awaiter(this, arguments, void 0, function* (type = "object") {
            // Load key.
            ...
            // Make request.
            const { body: status } = yield vlib.request({
                host: this.domain,
                endpoint: "/.status",
                method: "GET",
                params: { key },
    ...
    generate_tls_key(_a) {
        return __awaiter(this, arguments, void 0, function* ({ path, organization_unit = "IT", ec = true }) {
            // Args.
            if (path == null) {
    ...
    */
     ...
     */
    send_mail(_a) {
        return __awaiter(this, arguments, void 0, function* ({ sender = null, recipients = [], subject = null, body = "", attachments = [], }) {
            // Not enabled.
            ...
            // Send mail.
            try {
                return yield this.smtp.sendMail({
        ...
        const _render_mail_payment_line_item = ({ name, desc, unit_cost, quantity, total_cost, font_weight = "normal", divider = true, color = style.text_fg }) => {
            return [
                Table(TableRow(TableData(Text(name)
                    ...
                    .width("25%")
                    .margin_right(10), TableData(Text(desc)
                    .color(color)
                    .font_size(14)
                    .text_wrap("wrap")
                    .overflow_wrap("break-word")
                    ...
                    .font_weight(font_weight))
                    .width("35%")
                    .margin_right(10), TableData(Text(unit_cost)
                    .color(color)
                    .font_size(14)
                    .text_wrap("wrap")
                    .overflow_wrap("break-word")
                    ...
                    .font_weight(font_weight))
                    .fixed_width("13.32%")
                    .margin_right(10), TableData(Text(quantity)
                    .color(color)
                    .font_size(14)
                    .text_wrap("wrap")
                    .overflow_wrap("break-word")
                    ...
                    .font_weight(font_weight))
                    .fixed_width("13.32%")
                    .margin_right(10), TableData(Text(total_cost)
                    .color(color)
                    .font_size(14)
                    .text_wrap("wrap")
                    .overflow_wrap("break-word")
                    ...
                    .width("100%"),
                !divider ? null : TableRow(TableData(VStack()
        ...
        return [
            ...
            line_items.iterate_append((item, index) => {
                return Table(TableRow(TableData(Image(item.product.icon)
                    .frame(35, 35)
                    .margin_right(15))
                    .width("auto"), TableData(Table(Text(item.product.name)
                    ...
                    .width("100%"), TableData(Text(`${currency} ${item.subtotal.toFixed(2)}`)
                    .color(style.title_fg)
                    .font_size(14)
                    .font_weight("bold")
                    .margin(0)
            ...
            Table([
                ...
                return TableRow(TableData()
                    .width("100%"), TableData(Text(item[0])
                    .color(style.title_fg)
                    .font_size(14)
                    .ellipsis_overflow(true)
                    .font_weight("bold"))
                    .min_width(75), TableData(Text(item[1])
                    .color(style.title_fg)
                    .font_size(14)
                    .white_space("nowrap")
                    .font_weight("bold"))
        ...
        return this._mail_template({
            ...
            children: [
                ...
                TableRow(Text("We're delighted to inform you that your payment has been successfully processed. Thank you for your purchase.")
                    ...
                    .center()),
                // Image.
                TableRow(Image(`${this.full_domain}/vweb_static/payments/party.png`)
                    .frame(60, 60)
                    .margin(0, 0, 30, 0))
                    .center(),
                // Title.
        ...
        return this._mail_template({
            ...
            children: [
                ...
                TableRow(Text("We regret to inform you that your payment has encountered an issue and could not be processed successfully. We understand the inconvenience this may cause. Please try again, please contact customer support if the problem persists.")
                    ...
                    .center()),
                // Image.
                TableRow(ImageMask(`${this.full_domain}/vweb_static/payments/error.png`)
                    .frame(40, 40)
                    .mask_color("#E8454E")
                    .margin(0, 0, 30, 0))
                    .center(),
        ...
        return this._mail_template({
            ...
            children: [
                ...
                TableRow(Text("Your recent cancellation request has been successfully processed.")
                    ...
                    .center()),
                // Image.
                TableRow(Image(`${this.full_domain}/vweb_static/payments/check.png`)
                    .frame(40, 40)
                    .margin(0, 0, 30, 0))
                    .center(),
                // Title.
