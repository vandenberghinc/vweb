...
const tslib = {
     ...
     */
    _extract_macro_statements(path: string, data: string) : [string, Record<string, string>] {
        ...
        const iterator = new vhighlight.Iterator({
            ...
            callback(state) {
                ...
                if (is_preprocessor !== undefined && !state.is_preprocessor) {
                    ...
                    let s: any = is_preprocessor, mode = 0;
     ...
     */
    _fill_macro_statements(path: string, data: string, macros: Record<string, string>) : string {
...
if (require.main === module) {
    let source: string = "", error_limit: null | number = 25, debug_single_file: boolean = false, target_file: undefined | string = undefined;
